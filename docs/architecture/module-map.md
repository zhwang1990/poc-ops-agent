...
当前首轮重写页面包括登录页、Agent 工作台、Skill 注册中心和 SQL 工作台。页面必须优先消费真实控制面接口；未开放的提交、安装、升级、卸载、AI SQL 助手和 DML 执行能力只能显示为禁用状态。SQL 工作台 DML 执行仅允许 `dev`、`sit`、`uat`，生产环境只能查询。
...
- M09 操作台首轮原型化重写当前已经实现：
  ...
  - SQL 工作台接入连接目录、授权 Schema 元数据、校验接口和开发/测试环境受控单条 `SELECT` 查询结果，P2 DML 提交仅限 `dev`、`sit`、`uat`；
  - Playwright 已覆盖 `1280px`、`1440px`、`1920px` 三个桌面视口的浏览器验收。
- SQL 工作台 P1 切片已按开发/测试环境受控单条 `SELECT` 执行、DML 仅预检、Worker 出口 allowlist、凭据别名与结果治理边界完成验收；P2 生产 SQL 连接只能查询。
- SQL 工作台目标态以主流数据库客户端交互为基线，并通过右侧 AI SQL 助手提供错误分析和性能优化。P2 在不新增部署服务或模块编号的前提下，复用 M02、M05、M07 和 M09 开放 `dev`、`sit`、`uat` 受控 CRUD；生产 SQL 连接只能查询。
...
- AgentScope Java 已被决策为 P1 只读诊断目标主链路，当前已完成 ReAct 工具回调到平台执行器的只读闭环、评测解释和 Agent Runtime 进度事件切片：
  ...
  - 确定性单 Skill 只读入口保留为联调、兼容和紧急回退路径；AgentScope 主链路后续剩余项为真实模型供应方联调、远程 CI 门禁固化、集中审计存储、生产级 Worker 隔离演练和更完整评测集；
