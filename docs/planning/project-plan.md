...
| T010 实现执行上下文与不可篡改审计事件 | P1 验收完成 | 100% | 已实现请求级执行上下文、追加式文件审计持久化、最新审计查询入口，并验证主体、动作、资源、策略版本、TraceId、RequestId 和结果可追溯；现有自动化测试已覆盖 Bearer Token、浏览器会话鉴权成功、拒绝与审计读取链路；已补充 P1 文件审计保留、归档、恢复和访问控制运行手册；P1 已接受文件审计作为当前审计事实源；P2 已新增可选数据库审计主存储，统一 `AuditTrail` 可写入 `audit_event` 表并支持启动恢复快照 | 无 P1 阻塞；SIEM 接入、WORM 防篡改、组织级备份系统和真实环境恢复演练仍转入 P2/P3 |
...
| 模块 | 状态 | 进度 | 已完成 | 剩余条件 |
|---|---|---:|---|---|
| M03 Skill 契约与注册中心 | P1 验收完成 | 100% | 已落地 Skill Manifest、发布签名、注册和显式校验，并将 P1 只读 Skill 补足到 6 个；平台源包由 `backend/skills/<skill>/skill.package.yaml`、`schemas/` 和 `examples/` 目录生成到 `backend/contracts/skills/packages`，与 AgentScope `SKILL.md` 目录分离；P1 已接受该注册与发布校验闭环 | 无 P1 阻塞；发布流水编排、生产签名方案、契约包自动校验和更多 Worker 适配器转入 P2/P3 |
| M04 AgentScope 主运行链路 | P1 验收完成 | 100% | 已完成确定性候选筛选、发布态约束、Agent Runtime 模块边界、启用开关、未配置失败关闭、最终摘要 POC，并已将 AgentScope Java 决策为 P1 只读诊断目标主链路；M04 `AgentToolExecutor` 端口已携带 Runtime 身份、角色和 trace 上下文，AgentScope ReAct 已通过真实 `AgentTool` 回调平台执行器；Agent Tool 请求、完成和拒绝语义事件契约骨架、M05 事件发布接线、执行器级授权审计、多 Tool 幂等恢复演练、`POST /internal/routing/skills/explain` 路由解释 API、ReAct 单工具、多工具、注入拒绝和模型超时评测，以及动态模型供应方注册、API Key 加密持久化、默认供应方切换、受控连通性探测和运行时动态解析已补齐；P1 已接受该主链路作为只读诊断闭环 | 无 P1 阻塞；真实模型供应方联调、远程 CI 门禁固化、生产密钥管理联调、更完整评测集、集中审计和生产级 Worker 隔离演练转入 P2/P3 |
| M05 工作流与状态恢复 | P2 试点切片已交付 | 100% | 已生成强类型只读命令、短期 Worker 请求和顺序语义事件；同时已落地 H2/R2DBC 工作流实例、attempt 与事件持久化、幂等复用、结果与事件回读、启动恢复装配、版本化迁移脚本，以及针对 `FAILED_RETRYABLE` 和 attempt 已过期在途实例的单次受控重放；已新增 workflow-backed Agent Tool 执行器，服务端重算参数哈希、重做 M02 策略决策、写入 Tool Step、发布 Agent Tool requested/completed/rejected 语义事件、记录 Agent Tool 授权审计，并通过 WorkerGateway 提交只读命令；Agent workflow 终态幂等命中时不再重跑 Runtime，而是复用持久化的终态 `AgentTaskResult` 状态、摘要和 toolCallCount；P2 已交付 `sit` / H2 受控 DML 的数据库事务工作流、数据库审计、幂等复用和 `UNKNOWN_REQUIRES_HANDOFF` 禁止重放语义，受控 SQL DML 实施计划任务 5 的规划追溯已关闭 | `dev` / `uat` 真实数据库推广、外部密钥源联调和更长期恢复演练仍在 P2/P3 完成；生产保持只读 |
...
| M09 SQL 工作台 P2 受控 CRUD | `sit` / H2 试点切片已交付 | 75% | 已接受 ADR 0009；已交付 v1.1 服务端预检回执、环境和风险策略、数据库事务工作流与审计、幂等复用、未知结果人工接管、Worker 独立写凭据边界，以及 `sit` / `h2-local-test` 的 `INSERT`、`UPDATE`、`DELETE` 恰好一次 H2 E2E 验收；生产、能力或配置关闭、无效或过期回执/确认及未知结果均已验证不会重放写入 | 将同一门禁推广到 `dev` / `uat` 真实数据库，完成外部 KeyStore 和密钥源联调、环境演练与安全评审；生产继续只读 |
...
P1 SQL 工作台已按开发和测试环境受控单条 `SELECT` 执行、DML 仅预检的边界完成验收。`dev`、`sit`、`uat` 受控增删改查属于 P2 受控变更试点，必须在生产环境只允许查询的前提下，通过策略、持久化工作流、短事务、审计和 Worker 二次校验完成。

P2 发布中心以非生产 `dev`、`sit`、`uat` 环境为范围，提供 WAR 制品发布、Liberty 脚本 Profile 发布、启停、重启、回滚和只读日志分析。初始执行模型为单应用、单环境、多节点串行；`sit` 和 `uat` 默认需要二次确认或更严格审批；Liberty 发布不要求操作台上传制品，但必须由控制面页面维护并审核启用的通用脚本 Profile 调用成熟 JAR / HTTPS 发布能力，`dev`、`sit`、`uat` 可以引用同一个 Profile，不同节点通过服务器配置中的脚本参数传入 `serverName`、`applicationName`、`artifactPath` 等值，其中 `artifactPath` 引用以 `\\` 开头的共享目录制品路径；Tomcat 初始通过操作台上传 WAR 制品并使用可扩展策略执行。成功或失败必须由确定性状态检查判定，大模型只能基于脱敏日志提供诊断建议。

## P2 启动范围

首批 P2 工作聚焦两个低风险、可回滚、非生产受控变更切片：

- SQL 工作台非生产受控 CRUD：以 ADR 0009 为边界，只开放 `dev`、`sit`、`uat` 受控 `INSERT`、`UPDATE`、`DELETE`，无 `WHERE` 的 `UPDATE` / `DELETE` 必须二次确认，生产环境只能查询。
- 截至 2026-07-17，首个 `sit` / `h2-local-test` 受控 DML 试点切片已交付；该交付不代表 `dev` / `uat` 已启用。数据库事务工作流、统一数据库审计、幂等与未知结果人工接管已形成验收证据，受控 SQL DML 实施计划任务 5 的规划追溯关闭。
...
- 审计 P2 基础：统一审计链路已支持文件默认模式和数据库主存储模式；SQL 工作台与发布中心不得私有化审计表或绕过 `AuditTrail`，后续只允许在统一审计信封和强类型载荷上增量扩展。
...
- 当前实现状态：
  ...
  - 确定性单 Skill 只读工作流继续作为联调、兼容和紧急回退路径；AgentScope 主链路后续剩余项为真实模型供应方联调、远程 CI 门禁固化、集中审计存储、生产级 Worker 隔离演练和更完整评测集。
...
- 长期记忆后续仅作为独立设计议题评估，不阻塞 P2 的 SQL 工作台非生产受控 CRUD 和发布中心非生产受控变更。
