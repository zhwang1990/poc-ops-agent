...
export const helpSections = [
  ...
  },
  {
    ...
    summary: "说明 SQL 校验、非生产受控 CRUD 和生产只读的安全边界。",
    roleHints: ["开发运维", "数据库支持", "平台管理员"],
    relatedPages: ["SQL 工作区", "工作流事件", "审计记录"],
    boundary: "生产连接只能查询；P2 仅允许 dev、sit、uat 的低风险、可回滚受控变更能力。",
    keywords: ["SQL", "SQL 校验", "只读", "DML", "生产只读"],
    scenarios: [
      {
        ...
        whenToUse: "需要在授权连接上执行受控单条 SELECT 查询时使用。",
        prerequisites: ["连接目录中存在可见连接", "SQL 已通过只读校验", "查询不包含敏感输出"],
        steps: ["选择 dev、sit、uat 或 production 查询连接", "输入单条 SELECT", "运行校验", "确认后执行查询"],
        howToReadResult: ["结果仅代表当前连接", "分页和字段展示以服务端返回为准"],
        failureHandling: ["连接不可见时不要手写连接串", "校验失败时按报告修改 SQL"],
        safetyNotes: ["生产环境只能查询", "禁止在浏览器保存凭据"],
        keywords: ["SQL", "生产只读", "SELECT", "只读查询"],
      },
      {
  ...
  },
  {
    ...
    faqs: [
      ...
      {
        id: "why-production-read-only",
        title: "为什么生产环境只能查询？",
        summary: "P2 阶段禁止生产写执行；SQL 生产连接只能查询，发布中心生产环境仍不可见、不可配置、不可调用。",
        answer: "当前阶段只推进非生产、低风险、可回滚的受控变更试点。生产写能力需要新的设计决策、安全评审和验收证据。",
        keywords: ["生产只读", "生产环境", "P2", "发布中心", "SQL"],
      },
      {
